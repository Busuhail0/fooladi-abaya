...
def build_backup(connection,photos,max_bytes=20*1024*1024):
    ...
    names=sorted({row['photo'] for table in ('models','items') for row in tables[table] if row.get('photo')})
    if len(names)>100:raise CloudBackendError('Use administrative export for more than 100 photos.')
    raw=json.dumps({'format':'fooladi-cloud-v1','tables':tables},ensure_ascii=False).encode('utf-8')
